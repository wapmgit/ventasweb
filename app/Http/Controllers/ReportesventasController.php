...
//dd($request);
	...
	public function ventasarticulo(Request $request)
    {
        ...
        $empresa=DB::table('empresa')-> where('idempresa','=','1')->first();
            ...
            -> select(DB::raw('avg(dv.precio_venta/dv.cntgrp) as vpromedio'),DB::raw('sum(dv.cantidad*dv.cntgrp) as vendido'),'a.nombre',DB::raw('avg(dv.precio_venta/dv.cntgrp) as pventa'),'a.precio1 as pventa1','a.idarticulo','a.peso','ca.nombre as grupo')
            ->whereBetween('dv.fecha', [$query, $query2])
            ->where('ve.devolu','=',0)
		...
		if($request->get('opcion')>0){	
			...
			$datos=DB::table('detalle_venta as dv')            
             ...
             ->join ('articulos as a', 'a.idarticulo','=','dv.idarticulo') 
             ->join ('categoria as ca','ca.idcategoria','=','a.idcategoria')      
           -> select(DB::raw('avg(dv.precio_venta/dv.cntgrp) as vpromedio'),DB::raw('sum(dv.cantidad*dv.cntgrp) as vendido'),'a.nombre',DB::raw('sum(avg.precio_venta/dv.cntgrp) as pventa'),'a.precio1 as pventa1','a.idarticulo','a.peso','ca.nombre as grupo')

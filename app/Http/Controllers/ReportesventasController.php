...
//dd($request);
	...
	public function ventasarticulo(Request $request)
    {
        ...
        $empresa=DB::table('empresa')-> where('idempresa','=','1')->first();
            ...
            -> select(DB::raw('avg(dv.precio_venta/dv.cntgrp) as vpromedio'),DB::raw('sum(dv.cantidad*dv.cntgrp) as vendido'),'a.nombre',DB::raw('sum(dv.precio_venta*(dv.cantidad*dv.cntgrp)) as pventa'),'a.precio1 as pventa1','a.idarticulo','a.peso','ca.nombre as grupo')
            ->whereBetween('dv.fecha', [$query, $query2])
            ->where('ve.devolu','=',0)
	...
	public function reportecxcvencida(Request $request)
 {
		...
		public function ventasproveedor(Request $request)
    {
		...
		-> select(DB::raw('sum(dc.cantidad) as comprado'),'art.nombre','art.idarticulo','art.stock')
		->where('co.estatus','0')

...
public function recibo58($id){
            ...
            return view("ventas.venta.recibo58",["venta"=>$venta,"recibos"=>$recibo,"recibonc"=>$recibonc,"empresa"=>$empresa,"detalles"=>$detalles]);
...
public function notads($id){
			...
			$venta=DB::table('venta as v')
            -> join ('clientes as p','v.idcliente','=','p.id_cliente')
            -> select ('v.descuento','v.idventa','v.fecha_hora','v.fecha_emi','v.tasa','v.tasa','v.texe','v.base','v.total_iva','p.nombre','p.cedula',DB::raw('CONCAT(p.codpais,p.telefono) as telefono'),'p.direccion','v.control','v.tipo_comprobante','v.serie_comprobante','v.num_comprobante','v.impuesto','v.estado','v.total_venta','v.devolu')
            ->where ('v.idventa','=',$id)
            -> first();
			//dd($venta);
            $detalles=DB::table('detalle_venta as dv')
...
private function registrarMovimientoBancario($venta, $recibo, $moneda, $idCliente, $clinombre,$clicedula, $monto, $tmoneda)
{
    $mov = new MovBancos;

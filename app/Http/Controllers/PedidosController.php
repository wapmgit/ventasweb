...
class PedidosController extends Controller
{
   ...
   public function __construct()
	...
	public function create(Request $request){
		$rol=DB::table('roles')-> select('crearpedido','factsinexis')->where('iduser','=',$request->user()->id)->first();	
...
$q2=DB::table('articulos as art')
     ...
     if ($contador==""){$contador=0;}
      return view("pedidos.pedido.create",["categoria"=>$categoria,"rutas"=>$rutas,"seriales"=>$seriales,"rol"=>$rol,"personas"=>$personas,"articulos"=>$articulos,"monedas"=>$monedas,"contador"=>$contador,"empresa"=>$empresa,"vendedores"=>$vendedor]);
    } else { 
...
public function show(Request $request,$id){
   ...
   // inserta el recibo
        ...
        while($cont < count($idarticulo)){
			if($cantidad[$cont]>0){
				...
				if($articulo->comi){ $pcomiarti=$articulo->pcomision;
					...
					$pcomiarti=$dpedido->comision;
					$mcomiarti=(($cantidad[$cont]*$precio_venta[$cont])*($dpedido->comision/100));
    ...
    $kar->cantidad=$cantidad[$cont]*$usag[$cont];
	...
	if($ruta==0){ $c=">"; $idr=0;}else{$c="="; $idr=$ruta;}
		...
		->join('pedidos as ve','ve.idpedido','=','dv.idpedido')
		-> join('articulos as a','dv.idarticulo','=','a.idarticulo')
		-> select(DB::raw('sum(dv.cantidad) as cantidad'),DB::raw('sum(dv.cantidad*a.peso) as pesot'),DB::raw('sum(dv.precio_venta*dv.cantidad) as mventa'),'a.nombre as articulo','a.stock','a.idarticulo')
		->where('ve.devolu','=',0)
		->where('ve.impor','=',0)
	...
	public function addart(Request $request){
		$unidad="UND"; $cntgrp=1;
			if($request->usag>0){
				$opgrupo=Agrupados::findOrFail($request->usag);
				$unidad=$opgrupo->descripcion;	
				$cntgrp=$opgrupo->cantidad;		
				}
			$mytime=Carbon::now('America/Caracas');
	...
	public function recibo58p($id){
            ...
            return view("pedidos.pedido.recibo58",["venta"=>$venta,"recibos"=>$recibo,"recibonc"=>$recibonc,"empresa"=>$empresa,"detalles"=>$detalles]);
}
public function recibopedido($id){
            ...
            return view("pedidos.pedido.recibopedido",["venta"=>$venta,"recibos"=>$recibo,"recibonc"=>$recibonc,"empresa"=>$empresa,"detalles"=>$detalles]);
}
...
public function tcartap(Request $request, $id){
            ...
            $detalles=DB::table('detalle_venta as dv')
            -> join('articulos as a','dv.idarticulo','=','a.idarticulo')
            -> select('a.idarticulo','a.nombre as articulo','a.iva','dv.unidad','dv.cantidad','dv.descuento','dv.precio','dv.precio_venta')
            -> where ('dv.idventa','=',$id)
            ->get();
...
public function fbs($id){
	//dd($id);
            ...
            $detalles=DB::table('detalle_venta as dv')
            -> join('articulos as a','dv.idarticulo','=','a.idarticulo')
            -> select('a.idarticulo','dv.idarticulo','a.nombre as articulo','dv.unidad','a.codigo','a.iva','dv.cantidad','dv.descuento','dv.precio','dv.precio_venta')
            -> where ('dv.idventa','=',$id)
            ->get();
...
public function notabs($id){
            ...
            $detalles=DB::table('detalle_venta as dv')
            -> join('articulos as a','dv.idarticulo','=','a.idarticulo')
            -> select('a.idarticulo','dv.idarticulo','a.nombre as articulo','dv.unidad','a.codigo','a.iva','dv.cantidad','dv.descuento','dv.precio_venta','dv.precio')
            -> where ('dv.idventa','=',$id)
            ->get();

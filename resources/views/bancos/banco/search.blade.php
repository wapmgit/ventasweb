<div class="card">
        ...
        </div>	<form action="{{route('movcuentas')}}" method="POST" enctype="multipart/form-data" >         
			{{csrf_field()}}
...
<label>Hasta:</label>	 <input type="date" class="form-control" name="searchText2" value="">
	...
	<div class="form-group">
	<div class="input-group">
  	<label for="cliente">Cuenta de Clasificacion</label>
                    	 <select name="clasi" id="clasi" class="form-control selectpicker" data-live-search="true">
                    	 	<option value="0">Todos</option>
                           @foreach ($clasificador as $cta)
                           <option value="{{$cta -> idcod}}">{{$cta -> codigo}} ->  {{$cta -> descrip}} </option> 
                           @endforeach
                        </select>
	</div>
	</div>	

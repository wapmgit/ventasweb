...
<?php $mostrar=1; ?>
<div class="row">
		@include('reportes.pedido.revision.search')
</div>
...
$cefe=0;?>
 <!-- Main content -->
		<div class="invoice p-3 mb-3">
			...
			@include('reportes.pedido.revision.empresa')
              </div>
              <!-- /.row -->
					...
					<thead style="background-color: #E6E6E6">
						...
						<?php $total=0; $count=0; $mon=0; $cli=array();?>
							...
							<tr>
								...
								<td><small> <a href="{{route('pedidoanul',['id'=>$cat->idpedido])}}">   
								{{ $cat->idpedido}} </a></small></td>
								<td><small>{{ $cat->cedula}} {{ $cat->nombre}}</small></td>
								<td><small><small>{{ $cat->direccion}}</small></small></td>
								<td><small><?php echo date("d-m-Y",strtotime($cat->fechadevolu)); ?></small></td>
								<td>{{ $cat->userdevolu}}</td>
								<td>
								<?php $count++; $total=$total+$cat->total_venta; echo number_format( $cat->total_venta, 2,',','.')." $"; ?></td>
							</tr>
							@endforeach
							<tr><td colspan="4">Documentos: <?php echo $count; ?></td><td></td>
...
$(document).ready(function(){
  ...
  window.print(); 
  window.location="{{route('revisionpedidos')}}";
    });

...
<?php $mostrar=1; ?>
<div class="row">
		@include('reportes.articulos.inventariofecha.search')
</div>
...
<div class="row">
	<div class="col-md-12">
	  ...
	  ?> @include('reportes.articulos.inventariofecha.empresa')		
		 <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<table style="line-height:65%" border="1" width="98%" >
				<thead>
					<tr><td></td><td></td><td></td><td colspan="6" align="center" style="background-color: #E6E6E6" ><small>Resumen</small></td>
					</tr>
					<th><small>N°</small></th>
				...
				<?php  if($a->idarticulo==$in->idarticulo){ $ctin=$in->costop;$in_ant=$in->cantidad;} ?> 
				@endforeach
				@foreach ($anteriorout as $ou) 
				<?php if($a->idarticulo==$ou->idarticulo){$ctout=$ou->costop; $out=$ou->cantidad;} ?> 
				@endforeach			
					<tr>
				  <td><?php echo "<font size='2'>".$num."</font>";?></font></td>
				  <td><font size="1">{{$a->codigo}}</font></td>
				  <td width="50%"><font size="2">{{$a->nombre}}</font></td>
				  <td><font size="2"><?php if($ctin>0){$ctant=$ctin;}else{$ctant=$ctout;} 
				  echo number_format(($in_ant-$out), 2,',','.'); $acum_eant=$acum_eant+($in_ant-$out); ?></font></td>
				 <td>@foreach ($entrada as $m)  
				 <?php  if(($a->idarticulo==$m->idarticulo)) { $contingreso++; $ingresoscosto=$ingresoscosto+$m->costop; 
				 $aux=$m->cantidad;  }  ?>
					@endforeach 
				 <?php  if($contingreso>0){$promcmes=$ingresoscosto;}else{$promcmes=$ctant;} 
				 echo "<font size='2'>".number_format($aux, 2,',','.')."</font>"; $acum_in=$acum_in+$aux; ?></td>
			...
			<td><?php echo "<font size='1'><b>".number_format(($acumctra), 2,',','.')."</b></font>"; ?></td>
			<td></td>

	...
	<font size="4"><b>Documento:  <?php $idv=$venta->num_comprobante; echo add_ceros($idv,$ceros); ?> </b></font> </br>
	  <font size="2"> <b>  <?php echo date("d-m-Y h:i:s a",strtotime($venta->fecha_hora)); ?></b></font>
                 ...
                 <table style="line-height:90%"  id="tablecentro" class="tabla-secundaria">
                      ...
                      <tbody>
  @foreach($detalles as $det) <?php $acumv=$acumv+(($det->cantidad*$det->precio_venta)*$venta->tasa);?>
                        <tr height="10px"> 						
                         <td align="left"><span class="lista">
						   {{$det->cantidad}} {{$det->unidad}}   -
						  <?php echo strtolower($det->articulo);?><?php if($det->iva>0){echo "(G)"; }else { echo "(E)"; } ?> - <?php echo number_format( $det->precio_venta*$venta->tasa, 2,',','.'); ?> </span></td>                       
                          <td><span class="lista"><?php echo "Bs ".number_format( (($det->cantidad*$det->precio_venta)*$venta->tasa), 2,',','.'); ?></span></td>
                        </tr>
                        @endforeach
                      </tbody>
					     <tfoot>  
					  <th colspan="2" ><div align="center"><font size="4">        $: <?php echo number_format($venta->total_venta, 2,',','.'); ?><-->
                 Bs: <?php echo number_format(($acumv), 2,',','.'); ?></font></div></th>
                          </tfoot>
                  </table>

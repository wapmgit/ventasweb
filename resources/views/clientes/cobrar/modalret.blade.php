 <div class="modal fade" id="retencion{{$cat->idventa}}">
	  <form action="{{route('apliret')}}" method="get" enctype="multipart/form-data" >    
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title">Retencion en venta Doc->{{$cat->idventa}}</h4>
	...
	<input type="date" name="fecharet" value="<?php echo date("Y-m-d");?>" class="form-control">
				...
				<div class="col-lg-3 col-sm-3 col-md-3 col-xs-3">
            		 <div class="form-group">
            			...
            			<input type="text" name="miva" id="miva{{$cat->idventa}}" required value="{{$cat->ivabs}}"  class="form-control">
            		</div>
            	</div>
				<div class="col-lg-3 col-sm-3 col-md-3 col-xs-3">
            		 <div class="form-group">
...
function trunc (x, posiciones = 0) {
  ...
  return Number(numStr)
	...
	$("#sendretf<?php echo $cat->idventa; ?>").on("click",function(){
  	document.getElementById('sendretf<?php echo  $cat->idventa; ?>').style.display="none"; 
});	
	</script>
@endpush
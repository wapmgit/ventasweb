...
<div class="row">
 ...
 <small> (Ley de IVA-Art. 11  La administracion tributaria podra designar como responsables</br>
  ...
  <td>{{$retencion->mexento}}</td>
  <td>{{$retencion->mbase}}</td>
  <td>16</td>
  <td>{{$retencion->miva}}</td>
  <td>{{$retencion->ret}}</td>
  <td>{{$retencion->mret}}</td>
  </tr>
  <tr><td colspan="13" align="right">TOTAL A PAGAR: </td><td>{{$retencion->mret}}</td></tr>
  </table>
	  </div>
	   <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
...
<div class="row">
  ...
  <tr><td align="center">{{$retencion->codtrib}}</td>
  <td><?php echo date("d/m/Y",strtotime($retencion->fecha)); ?></td>
  <td align="center"><small>{{$retencion->descrip}}</small></td>
  <td align="center">{{$retencion->documento}}</td>
  <td align="center">{{$retencion->serie_comprobante}}</td>
  <td>&nbsp; </td>    
  <td align="center">{{$retencion->mfac}}</td>
  <td align="center">{{$retencion->mbase}}</td>
  <td align="center">{{$retencion->ret}} %</td>
  <td align="center">{{$retencion->sustraend}}</td>  
  <td align="center">{{$retencion->mret}}</td>
  </tr>
...
$(document).ready(function(){
	...
	window.print(); 
	window.location="{{route('retenciones')}}";
    });
